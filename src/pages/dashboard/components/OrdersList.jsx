          ...
          {processing ? (
            ...
            orders.map(({ customer, date, id, method, status, total }, _i) => (
              <tr key={_i} className="bg-white border-b hover:bg-tertiary text-primary text-[15px] text-nowrap">
                ...
                <TableColumn value={method} />
                <TableColumn value={<span className="text-yellow-primary">{status}</span>} />
                <TableColumn
                  value={
...
const NoDataFound = () => {
  return (
    <td colSpan={12}>
      <div className="flex items-center justify-center py-3 italic text-sm">No order found.</div>
    </td>
  );
};
